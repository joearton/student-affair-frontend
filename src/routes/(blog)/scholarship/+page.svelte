<script lang="ts">
  ...
  function update_offsets() {
    if (response_all.previous) {
      previous_offset =
        Number(new URL(response_all.previous).searchParams.get("offset")) || 0;
    }

    if (response_all.next) {
      next_offset =
        Number(new URL(response_all.next).searchParams.get("offset")) || 0;
    }
  }
...
<div class="container mb-5">
  <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-5 my-3">
    {#each ongoing_scholarship as scholarship, index}
      <a
        class="sch-ongoing-item"
        href={`/scholarship/${slugify(scholarship.name)}---${scholarship.code}`}
      >
...
<div class="container">
      ...
      <div class="my-3">
        <div class="input-group">
          <input
            ...
            onchange={() => update_scholarship()}
          />
          <button
            class="btn btn-primary"
            type="button"
            onclick={() => update_scholarship()}
            aria-label="Search"
          >
          ...
          <div class="col-md-3">
            ...
            <div class="form-check">
              <input
                ...
                type="checkbox"
                id="target1"
                bind:group={filter_target}
                onchange={() => update_scholarship()}
              />
              <label class="form-check-label" for="target1"
                >Mahasiswa Baru</label
              >
            </div>
            <div class="form-check">
              <input
                class="form-check-input"
                value="on-going-student"
                type="checkbox"
                id="target2"
                bind:group={filter_target}
                onchange={() => update_scholarship()}
          ...
          <div class="col-md-3">
            ...
            <div class="form-check">
              <input
                ...
                type="checkbox"
                id="source1"
                bind:group={filter_source}
                onchange={() => update_scholarship()}
              />
              <label class="form-check-label" for="source1"
                >Beasiswa Internal</label
              >
            </div>
            <div class="form-check">
              <input
            ...
            <div class="form-check">
              <input
                ...
                onchange={() => update_scholarship()}
              />
              <label class="form-check-label" for="destination2"
                >Perguruan Tinggi Lain</label
              >
            </div>
          </div>
  ...
  <div class="mb-3">
    {#if scholarships.length === 0}
      ...
      {/if}
    {/if}
    {#each scholarships.filter((s) => filter_status.includes("closed") || s.status !== "closed") as scholarship, index}
      <div
        class="sch-item d-block bg-white border-0 shadow-sm my-5"
      ...
      >
        <div class="row g-0">
          <div class="col-md-3">
            <div
              class="h-100"
              style="background-image: url('{scholarship.thumbnail}'); background-size: cover;"
            ></div>
          </div>
          <div class="col-md-9">
      ...
      <ul class="pagination">
        {#if response_all.previous}
          <li class="page-item">
            <button
              class="page-link"
              onclick={() => update_scholarship(previous_offset)}
              >Previous</button
            >
          </li>
        {/if}
        {#each Array(Math.ceil(response_all.count / response_all.limit)).fill(0) as _, pageIndex}
          <li
            class="page-item {pageIndex * response_all.limit ===
            response_all.offset
              ? 'active'
              : ''}"
          >
            <button
              class="page-link"
              onclick={() => update_scholarship(pageIndex * response_all.limit)}
              >{pageIndex + 1}</button
            >
          </li>
        {/each}
        {#if response_all.next}
          <li class="page-item">
            <button
              class="page-link"
              onclick={() => update_scholarship(next_offset)}>Next</button
            >
          </li>
        {/if}
      </ul>
